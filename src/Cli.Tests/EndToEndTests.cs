    ...
    : VerifyBase
{
    ...
    public void TestVerifyAuthenticationOptions()
    {
        string[] initArgs = { "init", "-c", TEST_RUNTIME_CONFIG_FILE, "--database-type", "mssql",
            "--auth.provider", "AzureAD", "--auth.audience", "aud-xxx", "--auth.issuer", "issuer-xxx", "--auth.validate-issuer", "validate-issuer-xxx" };
        Program.Execute(initArgs, _cliLogger!, _fileSystem!, _runtimeConfigLoader!);
        ...
        Assert.AreEqual("validate-issuer-xxx", runtimeConfig.Runtime.Host.Authentication?.Jwt?.ValidateIssuer);
    }
